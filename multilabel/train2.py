...
from defs import LmdbObj, MoEx1d, PrjImg, OnlineMeter, VisualBert, random_delete, random_modify, random_replace, mutex_transform, sequential_transform, delete_words, replace_hidden, shuffle_title, get_lxmert
...
class LXMERT(nn.Module):
    def __init__(self, ncls, nlayer=None):
        ...
        self.lxmert = get_lxmert('youzanai/bert-product-title-chinese', nlayer)
...
def train_lxmert(fd):
    ...
    m = MultiLabelLearner(LXMERT(len(prop)+1, (4, 3, 3)), partial(T.optim.AdamW, lr=2e-5, weight_decay=1e-6), [nn.BCEWithLogitsLoss()], amp=True)
    _, val_log = m.fit(MultiLabelDataset(train_obj, feature_db, prop, trans, 5), EPOCHS, 200, [(0, 'online', OnlineMeter(len(prop_ncls)))], 
